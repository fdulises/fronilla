...
class lieditor {
    constructor({ sel, inputs = lieditorinput}) {
        this.inputs = inputs;
        this.container = document.querySelector(sel);
        this.textarea = this.container.querySelector("textarea");
        this.container.classList.add('lieditor-cont');

        //Insertamos el contenteditable y ocultamos el textarea
        this.textarea.setAttribute("data-state", "hidden");
        this.container.insertBefore(this.createEditor(this.textarea.value), this.textarea);
        this.editable = this.container.querySelector(".lieditor-area");

        //Generamos y agregamos el toolbar
        this.container.insertBefore(this.createToolbar(), this.editable);

        this.floating();
        lieditor.comand('styleWithCSS', true);
    }

    //Definimos funcionamiento del metodo update
    value() {
        if (this.textarea.getAttribute("data-state") == "hidden")
            this.textarea.value = this.editable.innerHTML;
    ...
    createEditor(html) {
        ...
        editor.setAttribute("class", "lieditor-area");
    ...
    createToolbar() {
        ...
        el.setAttribute("class", "lieditor-toolbar");
        Object.keys(this.inputs).forEach(k => {
            ...
            ael.addEventListener(v.event, e => v.action({editor: this, input: ael}));
            el.appendChild(ael);
        });
        return el;
    }

    //Metodo que provee los comandos de edición
    ...
    //Metodo para flotar contenido
    float(d) {
        this.comand('insertHTML', '<div style="float:' + d + '">' + this.getSelection() + '</div>');
    }
    floating() {
...
let lieditorinput = {
    ...
    italic: {
        class: 'btn', type: 'button', event: 'click',
        action: e => lieditor.comand('italic', ''),
        title: 'Italica',
    ...
    strike: {
        class: 'btn', type: 'button', event: 'click',
        action: e => lieditor.comand('strikethrough', ''),
        title: 'Techado',
    ...
    underline: {
        class: 'btn', type: 'button', event: 'click',
        action: e => lieditor.comand('underline', ''),
        title: 'Subrayado',
    ...
    eraser: {
        class: 'btn', type: 'button', event: 'click',
        action: e => lieditor.comand('removeFormat', ''),
    ...
    formatblock: {
        ...
        action: e => {
            if (e.input.value == "<p>")
                lieditor.comand('insertHTML', '<p>' + lieditor.getSelection() + '</p>');
            else lieditor.comand('formatBlock', e.input.value);
            e.input.value = "";
        },
        title: 'Formatblock',
    ...
    aleft: {
        class: 'btn', type: 'button', event: 'click',
        action: e => lieditor.comand('justifyLeft', ''),
    ...
    acenter: {
        class: 'btn', type: 'button', event: 'click',
        action: e => lieditor.comand('justifyCenter', ''),
    ...
    aright: {
        class: 'btn', type: 'button', event: 'click',
        action: e=>lieditor.comand('justifyRight', ''),
    ...
    afull: {
        class: 'btn', type: 'button', event: 'click',
        action: e =>lieditor.comand('justifyFull', ''),
        title: 'Justificar',
    ...
    img: {
        class: 'btn', type: 'button', event: 'click',
        action: e =>{
            let selection = lieditor.getSelection(), url;
            if (lieditor.validURL(selection)) url = selection;
            else url = prompt("Ingrese URL de la imagen");
            if (url) lieditor.comand('insertImage', url);
        },
    ...
    ytvid: {
        ...
        action: e =>{
            ...
            if (url) {
                let ht = '<iframe width="650" height="405" src="https://www.youtube.com/embed/' + lieditor.youtubeId(url) + '" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>\n';
                lieditor.comand('insertHTML', ht);
            }
        },
    ...
    link: {
        ...
        action: e =>{
            ...
            if (url) {
                ...
                lieditor.comand('insertHTML', url);
            }
        },
    ...
    unlink: {
        class: 'btn', type: 'button', event: 'click',
        action: e=>lieditor.comand('unlink', ''),
        title: 'Quitar enlace',
    ...
    orderedlist: {
        class: 'btn', type: 'button', event: 'click',
        action: e => lieditor.comand('insertOrderedList', ''),
    ...
    unorderedlist: {
        class: 'btn', type: 'button', event: 'click',
        action: e => lieditor.comand('insertUnorderedList', ''),
    ...
    quote: {
        class: 'btn', type: 'button', event: 'click',
        action: e => lieditor.comand('formatBlock', "<blockquote>"),
        title: 'Insertar Cita',
    ...
    code: {
        class: 'btn', type: 'button', event: 'click',
        action: e=>{
            let selection = lieditor.getSelection(),
                hcode = selection ? selection : '<br>';
            hcode = "<pre><code>" + hcode + "</code></pre><p></p>";
            lieditor.comand('insertHTML', hcode);
        },
    ...
    changeview: {
        class: 'btn', type: 'button', event: 'click',
        action: e=>{
            if (e.editor.textarea.getAttribute("data-state") == "hidden") {
                e.editor.textarea.value = lieditor.formatHTML(lieditor.unformatHTML(e.editor.editable.innerHTML));
                e.editor.textarea.setAttribute("data-state", "showed");
                e.editor.editable.setAttribute("data-state", "hidden");
            } else {
                e.editor.editable.innerHTML = e.editor.textarea.value;
                e.editor.textarea.setAttribute("data-state", "hidden");
                e.editor.editable.setAttribute("data-state", "showed");
            }
        },
    ...
    fullscreen: {
        class: 'btn', type: 'button', event: 'click',
        action: e=>{
            if (e.editor.container.getAttribute("data-state")) {
                e.editor.container.removeAttribute("data-state");
            } else e.editor.container.setAttribute("data-state", "full");
        },
